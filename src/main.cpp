...
#define PIN_ONE_WIRE 17

#define server_send_interval_s 60

#define WIFI_AT_BOOT_TIMEOUT 30
...
int btnCick = false;
float temperature;
char hostname[]="hotwatercontroller";
bool last_send_ok_flag=false;

String serverName = "http://192.168.1.125/update_hot_water_temperature";
...
float get_temp()
{
  // uint32_t rand_source= esp_random(); // 32 bit word of random bits 
  // float temp=50.0*(rand_source/4294967295.0);
  // return temp;
...
void update_display()
{
  tft.setRotation(4); //Portrait with buttons at the bottom
  tft.setTextColor(TFT_GREENYELLOW);
  tft.fillScreen(TFT_BLACK);
  float hotness=(temperature-5.0)/45.0; // 0 -1 scale for hotness
  if (hotness<0)
  {
    hotness=0.0;
  }
  ...
  int rect_height=int(hotness*320.0); // Sensible range is 5-60  i.e. 45 degrees maps to 320 pixels
  // Red is 0xF800 i.e. top 5 bits
  // Blue is 0x001F i.e. bottom 5 bits
  int red=int(32*hotness);
  int blue=int(32*(1-hotness));
  uint32_t temp_color=(red*0x0800 | blue) & 0xFFFF;
  //Serial.printf("red = %d, blue = %d  =>",red,blue);
  ///Serial.println(temp_color,HEX);


  tft.fillRect(0,320-rect_height,240,rect_height,temp_color);
  tft.setTextSize(9);
  tft.setCursor(25,80);
  tft.printf("%d",(int)temperature);

  if (!last_send_ok_flag)
  {
    tft.setCursor(0,0);
    tft.setTextColor(TFT_RED);
    tft.setTextSize(3);
...
void loop()
{
    ...
    // }
    // button_loop();
    uint32_t next_send_time=millis()+1000*server_send_interval_s;
    ...
    if (httpResponseCode>0) {
      ...
      last_send_ok_flag=(httpResponseCode==200);
    }
    else {
      Serial.print("Error code: ");
      Serial.println(httpResponseCode);
      last_send_ok_flag=false;
    }
    // Free resources
    http.end();


    // Auto reconnect if wifi down by restarting...
    if (WiFi.status()!= WL_CONNECTED)
    {
      ESP.restart();
    }
